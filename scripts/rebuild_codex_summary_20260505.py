...
NOTE_TITLE = "20260505 Codex Summary"
NOTE_DATE = "2026-05-05"
NOTEBOOK = "Me"
TAB_DOMAIN = "Dev"
PRIMARY_DOMAIN = "me.dev"
DELETE_TITLES = ["(20260505) Codex Summary", "20260505 Codex Summary"]

SESSION_ROOT = Path("/Users/mac/.codex/sessions/2026/05/05")
LOCAL_TZ = timezone(timedelta(hours=8))
DAY_START_LOCAL = datetime(2026, 5, 5, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_END_LOCAL = datetime(2026, 5, 6, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_START_MS = int(DAY_START_LOCAL.timestamp() * 1000)
DAY_END_MS = int(DAY_END_LOCAL.timestamp() * 1000)

BLOCKS = [
    {
        "topic": "Notion Journal: added an Alternative Asset trade-thesis surface so gold, silver, oil, crypto, duration, and FX funding stress can be tracked alongside the equity book",
        "timeslot_title": "Codex: Alternative asset thesis tab",
        "domains": ["me.dev", "dev.nj"],
        "repo": "Notion Journal",
        "session_files": ["rollout-2026-05-05T17-31-04-019df77a-0e1a-7562-8ae7-349413316997.jsonl"],
        "request": "Add a dedicated Alternative Asset thesis area in the Investment module so non-equity macro trades have their own watchlist, rationale, and signal framing.",
        "summary": (
            "The Investment module was expanded beyond equities with a new Trade Thesis tab focused on alternative assets. "
            "The new surface tracks `XAU/USD`, `XAG/USD`, `WTI`, `BTC-USD`, `ETH-USD`, `TLT`, and `USD/JPY`, and frames them as linked macro instruments rather than isolated tickers. The implementation added the watchlist structure and supporting analysis copy so commodity, crypto, rates, and funding-stress ideas can be reviewed in one place."
        ),
        "outcome": (
            "Alternative-asset trade work now has a first-class place inside Notion Journal instead of being forced through stock-oriented thesis tabs."
        ),
        "files": [
    ...
    {
        "topic": "Notion Journal: added an iPhone investment shortcut popup that routes to market-specific tools based on Hong Kong trading hours",
        "timeslot_title": "Codex: Investment shortcut popup",
        "domains": ["me.dev", "dev.nj"],
        "repo": "Notion Journal",
        "session_files": ["rollout-2026-05-05T18-54-31-019df7c6-73dc-7b82-99f4-d550e81ef989.jsonl"],
        "request": "Give the crowded iPhone Investment module a shortcut popup that opens the right menu of tools for the current market session, especially U.S. hours after 20:30 HKT.",
        "summary": (
            "We added a new `Open Investment Shortcut` App Intent and wired it into the app shortcuts provider so the phone can jump directly into the right investment tools. "
            "The popup now chooses between U.S. and Hong Kong flows using Hong Kong time, defaulting to U.S. after `20:30 HKT` and before `05:00`, and to Hong Kong during its cash-session window. The menu options were also made market-specific so the shortcut opens practical destinations instead of a generic overloaded module."
        ),
        "outcome": (
            "On iPhone, the Investment module now has a faster entry point that matches the active trading session instead of making the user navigate a crowded screen manually."
        ),
        "files": [
            "Notion Journal/Notion_JournalApp.swift",
            "Notion Journal/UI/NJInvestmentModuleView.swift",
        ],
    },
    {
        "topic": "Notion Journal: cleaned up the future weekly scaffold note format, fixed its default font sizing, and repaired the existing placeholders already written into the local database",
        "timeslot_title": "Codex: Weekly scaffold cleanup",
        "domains": ["me.dev", "dev.nj"],
        "repo": "Notion Journal",
        "session_files": ["rollout-2026-05-05T22-44-26-019df898-f1ff-7372-97f2-0009755ae6be.jsonl"],
        "request": "Fix the new rolling `#WEEKLY` notes so they use normal body font and a clean one-line title instead of tiny text and awkward placeholder content.",
        "summary": (
            "The weekly scaffold generator was tightened up after the initial rollout. "
            "New future notes now start with a single visible line in the form `(YYYYMMDD) Weekday - Daily`, while `#WEEKLY` stays as metadata instead of leaking into the block body. The quick-note RTF encoder was updated to use the normal `17pt` body font, and the existing local placeholder notes were repaired in place so the already-created future scaffold does not stay inconsistent."
        ),
        "outcome": (
            "The rolling weekly scaffold now produces notes that look like normal journal entries and the previously generated placeholders were brought up to the same standard."
        ),
        "files": [
...
def main() -> None:
    ...
    txt_path = SOURCE_ROOT / "20260505-codex-source.txt"
    json_path = SOURCE_ROOT / "20260505-codex-source.json"
    pdf_path = SOURCE_ROOT / "20260505-codex-source.pdf"
    ...
    source_doc = {
        ...
        "primary_domain": PRIMARY_DOMAIN,
        "source": "Codex session history from local desktop session JSONL files for 2026-05-05",
        "blocks": [
            {
                "topic": b["topic"],
                "repo": b["repo"],
    ...
    existing_slots = cur.execute(
        ...
        """,
        ("%/Users/mac/.codex/sessions/2026/05/05/%",),
    ).fetchall()
    cur.execute(
        """
        ...
        WHERE owner_scope = 'ME' AND deleted = 0 AND title LIKE 'Codex:%' AND notes LIKE ?
        """,
        (latest_end, "%/Users/mac/.codex/sessions/2026/05/05/%"),
    )
