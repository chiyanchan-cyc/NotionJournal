...
NOTE_TITLE = "20260506 Codex Summary"
NOTE_DATE = "2026-05-06"
NOTEBOOK = "Me"
TAB_DOMAIN = "Dev"
PRIMARY_DOMAIN = "me.dev"
DELETE_TITLES = ["(20260506) Codex Summary", "20260506 Codex Summary"]

SESSION_ROOT = Path("/Users/mac/.codex/sessions/2026/05/06")
LOCAL_TZ = timezone(timedelta(hours=8))
DAY_START_LOCAL = datetime(2026, 5, 6, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_END_LOCAL = datetime(2026, 5, 7, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_START_MS = int(DAY_START_LOCAL.timestamp() * 1000)
DAY_END_MS = int(DAY_END_LOCAL.timestamp() * 1000)

BLOCKS = [
    {
        "topic": "Notion Journal: switched the investment quote path to Webull so the IRC popup and local quote cache stop relying on stale Yahoo or Stooq fields",
        "timeslot_title": "Codex: Webull quote integration",
        "domains": ["me.dev", "dev.nj"],
        "repo": "Notion Journal",
        "session_files": ["rollout-2026-05-06T10-30-24-019dfb1f-4858-7ce0-b067-11ddcaaf71db.jsonl"],
        "request": "Use the approved Webull OpenAPI quote source consistently, including the real Notion Journal investment popup, instead of falling back to older quote providers.",
        "summary": (
            "We traced the finance quote path end to end and found that the companion writer scripts were already using Webull, but the real app-side IRC popup was still reading Yahoo/Stooq-backed fields. "
            "The investment module was patched to read the local Webull quote cache directly, and Hong Kong rows in `Auto` or `Webull` mode now prefer Webull for current price, previous close, open, intraday range, volume, and day-change fields. That closed the gap between the external quote refresh path and the actual UI the user relies on."
        ),
        "outcome": (
            "The quote source is now coherent across the pipeline: refreshed data comes from Webull and the Notion Journal investment popup actually consumes that same source."
        ),
        "files": [
            "/Users/mac/Documents/New project/IRC/scripts/refresh_webull_quotes.py",
            "/Users/mac/Documents/New project/IRC/scripts/write_webull_quote_journal.py",
            "Notion Journal/UI/NJInvestmentModuleView.swift",
        ],
    },
    {
        "topic": "LLMJournal: removed stale widget-only cache fallbacks so dashboard and goal-review widgets stop showing impossible old totals after the app database is already clean",
        "timeslot_title": "Codex: Widget cache cleanup",
        "domains": ["me.dev", "dev.llm"],
        "repo": "LLMJournal",
        "session_files": ["rollout-2026-05-06T11-22-01-019dfb4e-8933-73c1-9a24-69f478bd492f.jsonl"],
        "request": "Figure out why the widget still shows absurd reading and piano totals even though the internal dashboard data is clean, and stop it from reviving stale cached values.",
        "summary": (
            "The widget bug turned out to be a source-of-truth problem rather than a math problem. "
            "The app dashboard rows and `doc_list` were already clean, but the widget extension could still fall back to its own stale private caches like `widget.ck.goalmetric...` and `widget.ck.dashboardsum...`. The widget code was tightened so Goal Review and dashboard surfaces use the cleaned shared mirror rows instead of resurrecting obsolete widget-local cache values."
        ),
        "outcome": (
            "The widget stack now aligns with the app database, so old inflated totals should stop reappearing after the primary dashboard has been corrected."
        ),
        "files": [
            "/Users/mac/Developer/LLMJournal/LLMJournalWidgetExtension/NJGoalReviewWidget.swift",
            "/Users/mac/Developer/LLMJournal/LLMJournalWidgetExtension/NJDashboardWidget.swift",
        ],
    },
    {
        "topic": "Notion Journal: stabilized the weekly scaffold and investment workspace by fixing sync pressure, cleaning the `#WEEKLY` domain routing, merging Heatlist into Macro Data, and repairing future-note tombstone behavior",
        "timeslot_title": "Codex: Weekly and investment cleanup",
        "domains": ["me.dev", "dev.nj"],
        "repo": "Notion Journal",
        "session_files": [
            "rollout-2026-05-06T11-53-19-019dfb6b-3231-7f11-9b5e-d489433ea9a0.jsonl",
            "rollout-2026-05-06T11-54-13-019dfb6c-0628-72e0-9399-124d8f6fe4a0.jsonl",
            "rollout-2026-05-06T13-27-46-019dfbc1-a96f-7a80-9dc8-89eead881261.jsonl",
            "rollout-2026-05-06T14-01-23-019dfbe0-727b-7293-8870-d62045945912.jsonl",
        ],
        "request": "Untangle the buggy future `#WEEKLY` flow, clean up where those notes appear and how they sync, reduce the app pressure that was making journaling hang, and fold the Heatlist surface into the more useful Macro Data view.",
        "summary": (
            "This was the main maintenance and stabilization thread of the day. "
            "We found the app was doing a full CloudKit pull every four seconds instead of roughly every fifteen minutes, which made the trade-thesis workspace and journaling path feel unstable under failing finance entities. That pull cadence was corrected, and the trade-thesis refresh policy was re-scoped to post-close by default. In parallel, the future `#WEEKLY` note flow was cleaned up aggressively: weekly date-title notes were pulled out of the wrong finance domain, calendar targeting was hardened, future-note creation and reconstructed-weekly behavior were reconciled, and bad scaffold blocks were pushed back to CloudKit as soft-delete tombstones instead of being hard-deleted and silently reappearing. The investment UI was also simplified by folding Heatlist into Macro Data, and the Xcode warning pass fixed the key Swift-concurrency warnings that were worth addressing immediately."
        ),
        "outcome": (
            "The weekly scaffold is now much less brittle, the investment workspace is less cluttered, and the app is under far less sync pressure than it was at the start of the day."
        ),
        "files": [
            "Notion Journal/Sync/CloudSyncEngine.swift",
...
def main() -> None:
    ...
    txt_path = SOURCE_ROOT / "20260506-codex-source.txt"
    json_path = SOURCE_ROOT / "20260506-codex-source.json"
    pdf_path = SOURCE_ROOT / "20260506-codex-source.pdf"
    ...
    source_doc = {
        ...
        "primary_domain": PRIMARY_DOMAIN,
        "source": "Codex session history from local desktop session JSONL files for 2026-05-06",
        "blocks": [
            {
                "topic": b["topic"],
                "repo": b["repo"],
    ...
    existing_slots = cur.execute(
        ...
        """,
        ("%/Users/mac/.codex/sessions/2026/05/06/%",),
    ).fetchall()
    cur.execute(
        """
        ...
        WHERE owner_scope = 'ME' AND deleted = 0 AND title LIKE 'Codex:%' AND notes LIKE ?
        """,
        (latest_end, "%/Users/mac/.codex/sessions/2026/05/06/%"),
    )
