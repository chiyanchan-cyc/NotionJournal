...
NOTE_TITLE = "20260510 Codex Summary"
NOTE_DATE = "2026-05-10"
NOTEBOOK = "Me"
TAB_DOMAIN = "Dev"
PRIMARY_DOMAIN = "me.dev"
DELETE_TITLES = ["(20260510) Codex Summary", "20260510 Codex Summary"]

SESSION_ROOT = Path("/Users/mac/.codex/sessions/2026/05/10")
LOCAL_TZ = timezone(timedelta(hours=8))
DAY_START_LOCAL = datetime(2026, 5, 10, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_END_LOCAL = datetime(2026, 5, 11, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_START_MS = int(DAY_START_LOCAL.timestamp() * 1000)
DAY_END_MS = int(DAY_END_LOCAL.timestamp() * 1000)

BLOCKS = [
    {
        "topic": "Audio Focus: expanded Midnight Rhapsody references and improved drill timing controls for daily competition practice review",
        "timeslot_title": "Codex: Midnight Rhapsody drill updates",
        "domains": ["me.dev", "dev.af"],
        "repo": "Audio Focus",
        "session_files": ["rollout-2026-05-10T07-56-57-019e0f2c-3a9f-72c1-bc57-decb487fa2e9.jsonl"],
        "request": "Support richer Midnight Rhapsody practice review by adding another reference recording and giving the Listen tab both coarse and fine timing adjustment controls.",
        "summary": (
            "The practice-analysis workflow for Midnight Rhapsody was extended in two concrete ways. "
            "First, the seeded reference list now includes a third recording, `Breanna.m4a`, alongside the existing references, so comparison listening has broader musical context. Second, the Listen tab’s timing controls were upgraded from only fine adjustment to a two-scale setup with `-1s`, `-0.1s`, `+0.1s`, and `+1s`, which makes it practical to jump quickly and then trim segment boundaries precisely during drill review."
        ),
        "outcome": (
            "Audio Focus now supports a more usable daily drill workflow for this piece: more reference material to compare against and faster segment timing edits during practice review."
        ),
        "files": [
            "/Users/mac/Developer/Audio_Focus/audio_focus/Audio_Focus/MidnightRhapsodyDrillView.swift",
            "/Users/mac/Developer/Audio_Focus/audio_focus/Sound/MidnightRhapsody/Breanna.m4a",
            "/Users/mac/Developer/Audio_Focus/audio_focus/Audio_Focus.xcodeproj/project.pbxproj",
        ],
    },
    {
        "topic": "Notion Journal: repaired orphaned reconstructed `#WEEKLY` blocks by giving them hidden owner notes and startup CloudKit repair so iPhone can receive the normal note structure",
        "timeslot_title": "Codex: Weekly sync repair",
        "domains": ["me.dev", "dev.nj"],
        "repo": "Notion Journal",
        "session_files": ["rollout-2026-05-10T11-33-07-019e0ff2-226e-74a3-8e46-5af9ffede526.jsonl"],
        "request": "Investigate why locally edited reconstructed `#WEEKLY` content is visible on Mac but never reaches iPhone, and fix the data shape so weekly blocks sync without polluting the visible notebook UI.",
        "summary": (
            "The root cause was structural rather than UI-specific: reconstructed weekly blocks could exist as tags and blocks, but had lost their owning `note` and `note_block` rows, so iPhone never received a normal note graph to pull. "
            "The fix added a startup repair in `AppStore.swift` that scans live orphan `#WEEKLY` blocks, attaches each to the correct calendar day, reuses an existing daily note when present, otherwise creates a hidden owner note, creates the missing `nj_note_block` link, and requeues `note`, `note_block`, and `block` for CloudKit push. The steady-state future path was also corrected so new weekly blocks get a hidden `_SYSTEM` / `_system.weekly` owner note immediately, while normal note lists and date-range lists filter those system notes out."
        ),
        "outcome": (
            "Weekly reconstructed edits should now sync like ordinary notes because they finally have the owning note structure CloudKit expects, without exposing those helper notes in the visible notebook UI."
        ),
        "files": [
            "Notion Journal/AppStore.swift",
            "Notion Journal/UI/UINoteListView.swift",
...
def main() -> None:
    ...
    txt_path = SOURCE_ROOT / "20260510-codex-source.txt"
    json_path = SOURCE_ROOT / "20260510-codex-source.json"
    pdf_path = SOURCE_ROOT / "20260510-codex-source.pdf"
    ...
    source_doc = {
        ...
        "primary_domain": PRIMARY_DOMAIN,
        "source": "Codex session history from local desktop session JSONL files for 2026-05-10",
        "blocks": [
            {
                "topic": b["topic"],
                "repo": b["repo"],
    ...
    existing_slots = cur.execute(
        ...
        """,
        ("%/Users/mac/.codex/sessions/2026/05/10/%",),
    ).fetchall()
    cur.execute(
        """
        ...
        WHERE owner_scope = 'ME' AND deleted = 0 AND title LIKE 'Codex:%' AND notes LIKE ?
        """,
        (latest_end, "%/Users/mac/.codex/sessions/2026/05/10/%"),
    )
