...
NOTE_TITLE = "20260514 Codex Summary"
NOTE_DATE = "2026-05-14"
NOTEBOOK = "Me"
TAB_DOMAIN = "Dev"
PRIMARY_DOMAIN = "me.dev"
DELETE_TITLES = ["(20260514) Codex Summary", "20260514 Codex Summary"]

SESSION_ROOT = Path("/Users/mac/.codex/sessions/2026/05/14")
LOCAL_TZ = timezone(timedelta(hours=8))
DAY_START_LOCAL = datetime(2026, 5, 14, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_END_LOCAL = datetime(2026, 5, 15, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_START_MS = int(DAY_START_LOCAL.timestamp() * 1000)
DAY_END_MS = int(DAY_END_LOCAL.timestamp() * 1000)

BLOCKS = [
    {
        "topic": "Notion Journal: repaired cross-device weekly sync by fixing timestamp-poisoned pull skips, expiring stale editor leases, and adding a live CloudKit pull loop while iPhone stays open",
        "timeslot_title": "Codex: Weekly cross-device sync repair",
        "domains": ["me.dev", "dev.nj"],
        "repo": "Notion Journal",
        "session_files": ["rollout-2026-05-14T08-07-06-019e23ce-f65e-7711-9e40-e4ac9b837721.jsonl"],
        "request": "Fix the recurring case where an MBA weekly-note edit is visible locally but does not reach iPhone, and make sure iPhone pulls new CloudKit changes while the app remains open.",
        "summary": (
            "This was a full sync-path repair, not a one-off record tweak. "
            "The iPhone logs showed several distinct blockers layered together: future-timestamp repair could make the receiver think its local copy was newer and skip the MBA payload; payload-only edits could look `dirty_bl=0` while the real gate lived in `nj_dirty`; stale remote editor leases from the MacBook Air made iPhone refuse saves with “remote update detected”; and after cold start, iPhone only auto-pulled CloudKit changes every fifteen minutes, so a live-open app could simply miss fresh MBA pushes until restart. The fixes addressed each layer: pull rules now detect timestamp-repair state on attached future `#WEEKLY` blocks and allow sane CloudKit payloads to override it, reconstructed editors now expire their remote lease when editing truly ends instead of letting an inactive MBA look active, and `CloudSyncEngine` now runs a ten-second live-pull loop so an already-open iPhone asks CloudKit for fresh changes without requiring a relaunch."
        ),
        "outcome": (
            "MBA weekly edits should now have a viable path to iPhone while both devices stay open: stale repair timestamps no longer poison pull decisions, inactive editor leases should clear promptly, and iPhone performs ongoing live pulls instead of waiting for a cold-start sync."
        ),
        "files": [
...
def main() -> None:
    ...
    txt_path = SOURCE_ROOT / "20260514-codex-source.txt"
    json_path = SOURCE_ROOT / "20260514-codex-source.json"
    pdf_path = SOURCE_ROOT / "20260514-codex-source.pdf"
    ...
    source_doc = {
        ...
        "primary_domain": PRIMARY_DOMAIN,
        "source": "Codex session history from local desktop session JSONL files for 2026-05-14",
        "blocks": [
            {
                "topic": b["topic"],
                "repo": b["repo"],
    ...
    existing_slots = cur.execute(
        ...
        """,
        ("%/Users/mac/.codex/sessions/2026/05/14/%",),
    ).fetchall()
    cur.execute(
        """
        ...
        WHERE owner_scope = 'ME' AND deleted = 0 AND title LIKE 'Codex:%' AND notes LIKE ?
        """,
        (latest_end, "%/Users/mac/.codex/sessions/2026/05/14/%"),
    )
