...
NOTE_TITLE = "20260430 Codex Summary"
NOTE_DATE = "2026-04-30"
NOTEBOOK = "Me"
TAB_DOMAIN = "Dev"
PRIMARY_DOMAIN = "me.dev"
DELETE_TITLES = ["(20260430) Codex Summary", "20260430 Codex Summary"]

SESSION_ROOT = Path("/Users/mac/.codex/sessions/2026/04/30")
LOCAL_TZ = timezone(timedelta(hours=8))
DAY_START_LOCAL = datetime(2026, 4, 30, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_END_LOCAL = datetime(2026, 5, 1, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_START_MS = int(DAY_START_LOCAL.timestamp() * 1000)
DAY_END_MS = int(DAY_END_LOCAL.timestamp() * 1000)

BLOCKS = [
    {
        "topic": "Notion Journal: removed a SQLite hot-path bottleneck that made reconstructed `#weekly` views far slower than their result size justified",
        "timeslot_title": "Codex: Weekly reconstructed query fix",
        "domains": ["me.dev", "dev.nj"],
        "repo": "Notion Journal",
        "session_files": ["rollout-2026-04-30T10-29-06-019ddc37-ede4-7a12-9de3-5a5fb976d81c.jsonl"],
        "request": "Find out why `ReconstructedView` for `#weekly` became so slow and patch the real database/query problem instead of papering over it.",
        "summary": (
            "We traced the slowdown to a query shape that prevented SQLite from doing cheap tag lookups. "
            "The reconstructed tag path was using `lower(...)` predicates, missing a tag-side index, carrying an unnecessary debug count query, and forcing a worse join plan even though the weekly view only needed a handful of rows. The fix added a case-insensitive tag index, rewrote the predicates to use `COLLATE NOCASE`, removed the duplicate count query, and restored a cleaner primary-key join path for block lookup."
        ),
        "outcome": (
            "This was a focused database-path cleanup: the weekly reconstructed view should now behave like a small-result query instead of a full-table scan disguised as a convenience filter."
        ),
        "files": [
    ...
    {
        "topic": "Notion Journal: simplified the Investment macro workflow so the calendar focuses on important dates and event-level refresh requests become durable",
        "timeslot_title": "Codex: Macro calendar workflow",
        "domains": ["me.dev", "dev.nj"],
        "repo": "Notion Journal",
        "session_files": ["rollout-2026-04-30T20-46-04-019dde6c-c7e8-70f3-aff5-9c4595b54dda.jsonl"],
        "request": "Keep the Asia-close macro refresh moving, but also reshape the Investment module so daily moves live in Macro Data while the calendar stays focused on important dates and earnings-triggered follow-up.",
        "summary": (
            "This thread combined the daily macro note with a structural Investment-module cleanup. "
            "We wrote the April 30 Asia-close note with the latest session split across Asia and carried US dates, then changed the app so the calendar stops acting like a dumping ground for daily snapshot rows. Important dates and watchlist earnings remain on the calendar, while daily market movement belongs in the data view. To support that workflow, event rows gained a real persisted refresh-request signal plus an analysis-summary field, and the runbook was updated so future Codex refresh passes know how to consume and clear those requests. The session finished with the visible label rename from `Macro` to `Macro Calendar` and from `Macro Line View` to `Macro Data`."
        ),
        "outcome": (
            "The investment workflow is now cleaner conceptually: calendar for dates and catalysts, data view for market movement, and a durable hook for event reanalysis instead of relying on ad hoc copy-prompt loops."
        ),
        "files": [
            "Notion Journal/UI/NJInvestmentModuleView.swift",
            "Notion Journal/Model.swift",
            "Notion Journal/DB/DBSchemaInstaller.swift",
            "Notion Journal/DB/DBCalendarTable.swift",
            "Notion Journal/DB/DBNoteRepository.swift",
            "Notion Journal/Docs/Investment_Macro_Daily_Refresh_Automation.md",
            "Notion Journal/Docs/Investment Macro Daily Notes/Investment Macro Daily Refresh - 2026-04-30 Asia Close.md",
        ],
    },
]
...
def main() -> None:
    ...
    txt_path = SOURCE_ROOT / "20260430-codex-source.txt"
    json_path = SOURCE_ROOT / "20260430-codex-source.json"
    pdf_path = SOURCE_ROOT / "20260430-codex-source.pdf"
    ...
    source_doc = {
        ...
        "primary_domain": PRIMARY_DOMAIN,
        "source": "Codex session history from local desktop session JSONL files for 2026-04-30",
        "blocks": [
            {
                "topic": b["topic"],
                "repo": b["repo"],
    ...
    existing_slots = cur.execute(
        ...
        """,
        ("%/Users/mac/.codex/sessions/2026/04/30/%",),
    ).fetchall()
    cur.execute(
        """
        ...
        WHERE owner_scope = 'ME' AND deleted = 0 AND title LIKE 'Codex:%' AND notes LIKE ?
        """,
        (latest_end, "%/Users/mac/.codex/sessions/2026/04/30/%"),
    )
