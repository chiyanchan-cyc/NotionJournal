...
NOTE_TITLE = "20260507 Codex Summary"
NOTE_DATE = "2026-05-07"
NOTEBOOK = "Me"
TAB_DOMAIN = "Dev"
PRIMARY_DOMAIN = "me.dev"
DELETE_TITLES = ["(20260507) Codex Summary", "20260507 Codex Summary"]

SESSION_ROOT = Path("/Users/mac/.codex/sessions/2026/05/07")
LOCAL_TZ = timezone(timedelta(hours=8))
DAY_START_LOCAL = datetime(2026, 5, 7, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_END_LOCAL = datetime(2026, 5, 8, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_START_MS = int(DAY_START_LOCAL.timestamp() * 1000)
DAY_END_MS = int(DAY_END_LOCAL.timestamp() * 1000)

BLOCKS = [
    {
        "topic": "Journal for Mac: disabled the automatic video-archive path by default and tightened Photos access so background scans stop triggering metadata fetches on the main queue",
        "timeslot_title": "Codex: Video archive fix",
        "domains": ["me.dev", "dev.jm"],
        "repo": "Journal for Mac",
        "session_files": ["rollout-2026-05-07T06-43-00-019dff75-729f-79b3-9a2e-751dd3849983.jsonl"],
        "request": "Trace the `PHAssetOriginalMetadataProperties` warning on the Mac mini build and stop the background work that is needlessly scanning Photos video assets.",
        "summary": (
            "The noisy Photos warning was not coming from the active Xcode debug build at first, but from a separate distributed app instance still running on the Mac mini. "
            "After switching to the correct machine and process, the root cause was narrowed to the automatic video-archive scheduler and its idle drain path, which kept scanning Photos video assets and forcing metadata/resource lookups on the main queue. The fix was to make automatic video archive opt-in behind `JOURNAL_ENABLE_AUTOMATIC_VIDEO_ARCHIVE=1` and tighten the Photos-discovery path so the archive flow only runs when explicitly enabled."
        ),
        "outcome": (
            "Journal for Mac should stop doing surprise background Photos scans by default, which removes the metadata warning and reduces unnecessary idle work on the Mac mini."
        ),
        "files": [
            "/Users/mac/Developer/Journal for Mac/Journal for Mac/AppStore.swift",
            "/Users/mac/Developer/Journal for Mac/Journal for Mac/Common/NJVideoArchiveStore.swift",
        ],
    },
    {
        "topic": "Notion Journal: fixed investment-ledger integrity by separating mixed-currency totals, rejecting bad HK quotes, and re-enabling ledger CloudKit sync with startup backfill",
        "timeslot_title": "Codex: Ledger sync and quote fix",
        "domains": ["me.dev", "dev.nj"],
        "repo": "Notion Journal",
        "session_files": ["rollout-2026-05-07T14-35-53-019e0126-62ea-7470-9ce0-016334ae7389.jsonl"],
        "request": "Fix the broken investment ledger behavior: bad HK quotes were poisoning rows, USD and HKD were being added together, and Mac-to-iPhone ledger sync was effectively disabled.",
        "summary": (
            "This thread combined quote validation, portfolio math, and sync plumbing. "
            "The ledger summary was corrected so `USD` and `HKD` are no longer merged into one fake total, and the investment UI now rejects absurd HK quote values before they can flow into ledger calculations. At the sync layer, `investment_ledger_transaction` had CloudKit scaffolding but was explicitly disabled, so Mac-created rows would not actually propagate to iPhone. That disable was removed and a one-time startup backfill was added so existing local ledger rows can be pushed into the live sync graph."
        ),
        "outcome": (
            "The investment ledger should now produce defensible per-currency totals, ignore obviously broken quote inputs, and actually sync between Mac and iPhone instead of silently staying local."
        ),
        "files": [
            "Notion Journal/AppStore.swift",
            "Notion Journal/UI/NJInvestmentModuleView.swift",
            "Notion Journal/Sync/NJCloudSyncCoordinator.swift",
        ],
    },
]
...
def main() -> None:
    ...
    txt_path = SOURCE_ROOT / "20260507-codex-source.txt"
    json_path = SOURCE_ROOT / "20260507-codex-source.json"
    pdf_path = SOURCE_ROOT / "20260507-codex-source.pdf"
    ...
    source_doc = {
        ...
        "primary_domain": PRIMARY_DOMAIN,
        "source": "Codex session history from local desktop session JSONL files for 2026-05-07",
        "blocks": [
            {
                "topic": b["topic"],
                "repo": b["repo"],
    ...
    existing_slots = cur.execute(
        ...
        """,
        ("%/Users/mac/.codex/sessions/2026/05/07/%",),
    ).fetchall()
    cur.execute(
        """
        ...
        WHERE owner_scope = 'ME' AND deleted = 0 AND title LIKE 'Codex:%' AND notes LIKE ?
        """,
        (latest_end, "%/Users/mac/.codex/sessions/2026/05/07/%"),
    )
