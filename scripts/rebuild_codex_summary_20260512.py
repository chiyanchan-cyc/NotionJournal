...
NOTE_TITLE = "20260512 Codex Summary"
NOTE_DATE = "2026-05-12"
NOTEBOOK = "Me"
TAB_DOMAIN = "Dev"
PRIMARY_DOMAIN = "me.dev"
DELETE_TITLES = ["(20260512) Codex Summary", "20260512 Codex Summary"]

SESSION_ROOT = Path("/Users/mac/.codex/sessions/2026/05/12")
LOCAL_TZ = timezone(timedelta(hours=8))
DAY_START_LOCAL = datetime(2026, 5, 12, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_END_LOCAL = datetime(2026, 5, 13, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_START_MS = int(DAY_START_LOCAL.timestamp() * 1000)
DAY_END_MS = int(DAY_END_LOCAL.timestamp() * 1000)

BLOCKS = [
    {
        "topic": "Notion Journal: corrected reconstructed weekly visibility and fixed a startup sync gap where already-dirty rows could sit forever without a push attempt",
        "timeslot_title": "Codex: Reconstructed weekly sync fix",
        "domains": ["me.dev", "dev.nj"],
        "repo": "Notion Journal",
        "session_files": ["rollout-2026-05-12T07-25-58-019e195c-94b4-77c1-a9f2-0a1689db70f9.jsonl"],
        "request": "Fix two weekly-note issues: past scaffold rows should hide correctly in ReconstructedView, and dirty weekly rows already queued at launch should actually get pushed to CloudKit.",
        "summary": (
            "This thread fixed both a presentation bug and a sync trigger gap in the weekly workflow. "
            "On the UI side, `ReconstructedView` was only treating rows containing `daily focus` as hideable past-day rows, so the newer scaffold titles like `(20260510) Sunday - Daily` stayed visible even when `Show past days` was off. That filter was widened so the new title shape is recognized correctly while year and month markers remain visible. On sync, the deeper issue was that some `nj_dirty` rows were already queued with `attempts=0`, but `CloudSyncEngine.start()` only scheduled pushes after new dirty notifications. The startup path was patched so, after initial pull, any existing pending dirty rows trigger a push instead of waiting indefinitely."
        ),
        "outcome": (
            "Past daily scaffold rows now hide the way the weekly view expects, and weekly-note repairs that are already dirty at app launch should actually leave the Mac instead of stalling silently."
        ),
        "files": [
            "Notion Journal/Notes/NoteEditor/Rescontruct/NJReconstructedNoteView.swift",
            "Notion Journal/Sync/CloudSyncEngine.swift",
        ],
    },
    {
        "topic": "Notion Journal: added undo directly to the Proton floating bar and tightened focus routing so reconstructed editors reclaim the real text responder reliably",
        "timeslot_title": "Codex: Proton undo and focus fix",
        "domains": ["me.dev", "dev.nj"],
        "repo": "Notion Journal",
        "session_files": ["rollout-2026-05-12T10-46-50-019e1a14-7d26-7bd0-9235-6fa3dd01537f.jsonl"],
        "request": "Add an undo control to the Proton floating format bar for touch workflows, and fix the focus leak where tapping an already-focused reconstructed block failed to reclaim the actual editor responder.",
        "summary": (
            "The floating editing toolbar gained a direct undo affordance for non-keyboard use, backed by the underlying `UITextView` undo manager and mirrored support inside collapsible-section editors. "
            "The second part of the work fixed a focus-resolution bug in reconstructed notes: tapping a block that looked focused did not always restore the actual text responder because the floating bar could fall back to a global first responder from another note. The focus routing was tightened so the tapped block explicitly reclaims `protonHandle.focus()`, and the floating bar now prefers its own view-local focused handle before consulting global responder state."
        ),
        "outcome": (
            "Touch editing in Proton now has a practical undo button, and reconstructed-note editing should feel much less flaky because toolbar actions stay attached to the block you actually tapped."
        ),
        "files": [
            "Notion Journal/Notes/NoteEditor/NJProtonFloatingFormatBar.swift",
...
def main() -> None:
    ...
    txt_path = SOURCE_ROOT / "20260512-codex-source.txt"
    json_path = SOURCE_ROOT / "20260512-codex-source.json"
    pdf_path = SOURCE_ROOT / "20260512-codex-source.pdf"
    ...
    source_doc = {
        ...
        "primary_domain": PRIMARY_DOMAIN,
        "source": "Codex session history from local desktop session JSONL files for 2026-05-12",
        "blocks": [
            {
                "topic": b["topic"],
                "repo": b["repo"],
    ...
    existing_slots = cur.execute(
        ...
        """,
        ("%/Users/mac/.codex/sessions/2026/05/12/%",),
    ).fetchall()
    cur.execute(
        """
        ...
        WHERE owner_scope = 'ME' AND deleted = 0 AND title LIKE 'Codex:%' AND notes LIKE ?
        """,
        (latest_end, "%/Users/mac/.codex/sessions/2026/05/12/%"),
    )
