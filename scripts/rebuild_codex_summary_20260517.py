...
NOTE_TITLE = "20260517 Codex Summary"
NOTE_DATE = "2026-05-17"
NOTEBOOK = "Me"
TAB_DOMAIN = "Dev"
PRIMARY_DOMAIN = "me.dev"
DELETE_TITLES = ["(20260517) Codex Summary", "20260517 Codex Summary"]

SESSION_ROOT = Path("/Users/mac/.codex/sessions/2026/05/17")
LOCAL_TZ = timezone(timedelta(hours=8))
DAY_START_LOCAL = datetime(2026, 5, 17, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_END_LOCAL = datetime(2026, 5, 18, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_START_MS = int(DAY_START_LOCAL.timestamp() * 1000)
DAY_END_MS = int(DAY_END_LOCAL.timestamp() * 1000)

BLOCKS = [
    {
        "topic": "Journal for Mac: fixed Mac mini RAW export scheduling so the headless backend actually installs five-minute timers instead of being paused by foreground-focus logic",
        "timeslot_title": "Codex: RAW export scheduler fix",
        "domains": ["me.dev", "dev.jm"],
        "repo": "Journal for Mac",
        "session_files": ["rollout-2026-05-17T07-55-38-019e3337-8963-7013-8936-d1a282d34457.jsonl"],
        "request": "Investigate why RAW export is not firing every five minutes on the Mac mini and fix the scheduler so the headless backend actually runs on that cadence.",
        "summary": (
            "The RAW export scheduler on the Mac mini had three separate blockers. "
            "The saved preference had RAW disabled, the default interval was still `1800` seconds instead of the intended five-minute cadence, and the headless backend was entering the normal UI `start()` path, which could mark the app as foreground-focused and skip `scheduleAll()`, so timers never got installed. The fix made headless startup bypass foreground-focus pause behavior, ensured RAW timers are invalidated correctly during focus suspension, and changed the RAW default and UI placeholder to `300` seconds."
        ),
        "outcome": (
            "On the Mac mini, RAW export should now schedule and run on the intended five-minute loop when enabled, instead of silently idling because the backend never installs its timers."
        ),
        "files": [
            "/Users/mac/Developer/Journal for Mac/Journal for Mac/Backend/BackendService.swift",
            "/Users/mac/Developer/Journal for Mac/Journal for Mac/Backend/BackendSettings.swift",
            "/Users/mac/Developer/Journal for Mac/Journal for Mac/ContentView.swift",
        ],
    },
]
...
def main() -> None:
    ...
    txt_path = SOURCE_ROOT / "20260517-codex-source.txt"
    json_path = SOURCE_ROOT / "20260517-codex-source.json"
    pdf_path = SOURCE_ROOT / "20260517-codex-source.pdf"
    ...
    source_doc = {
        ...
        "primary_domain": PRIMARY_DOMAIN,
        "source": "Codex session history from local desktop session JSONL files for 2026-05-17",
        "blocks": [
            {
                "topic": b["topic"],
                "repo": b["repo"],
    ...
    existing_slots = cur.execute(
        ...
        """,
        ("%/Users/mac/.codex/sessions/2026/05/17/%",),
    ).fetchall()
    cur.execute(
        """
        ...
        WHERE owner_scope = 'ME' AND deleted = 0 AND title LIKE 'Codex:%' AND notes LIKE ?
        """,
        (latest_end, "%/Users/mac/.codex/sessions/2026/05/17/%"),
    )
