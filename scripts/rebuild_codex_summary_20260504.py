...
NOTE_TITLE = "20260504 Codex Summary"
NOTE_DATE = "2026-05-04"
NOTEBOOK = "Me"
TAB_DOMAIN = "Dev"
PRIMARY_DOMAIN = "me.dev"
DELETE_TITLES = ["(20260504) Codex Summary", "20260504 Codex Summary"]

SESSION_ROOT = Path("/Users/mac/.codex/sessions/2026/05/04")
LOCAL_TZ = timezone(timedelta(hours=8))
DAY_START_LOCAL = datetime(2026, 5, 4, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_END_LOCAL = datetime(2026, 5, 5, 0, 0, 0, tzinfo=LOCAL_TZ)
DAY_START_MS = int(DAY_START_LOCAL.timestamp() * 1000)
DAY_END_MS = int(DAY_END_LOCAL.timestamp() * 1000)

BLOCKS = [
    {
        "topic": "Notion Journal: turned Industry Analysis into a pre-thesis market map with clearer industry roles and constituent watchlists",
        "timeslot_title": "Codex: Industry map restructure",
        "domains": ["me.dev", "dev.nj"],
        "repo": "Notion Journal",
        "session_files": ["rollout-2026-05-04T19-35-15-019df2c5-62b6-7c61-9805-f07be0aa2361.jsonl"],
        "request": "Stop making Industry Analysis repeat existing trade theses. Make it a pre-thesis market and industry map with real industry roles and constituent watchlists.",
        "summary": (
            "The Industry Analysis surface was rewritten around discovery instead of conclusion. "
            "We added explicit `mapRole` structure to the industry themes, replaced thesis-like copy with a broader market-by-market industry map, renamed the old watch language to `Interest Signals`, and renamed the symbol section to `Constituent Watchlist` so the screen behaves like a scouting surface before any trade thesis exists."
        ),
        "outcome": (
            "Industry Analysis now supports upstream research properly: it frames what each industry is for, what to watch, and which constituents matter before the user commits to a named thesis."
        ),
        "files": [
    ...
    {
        "topic": "Notion Journal: removed the calendar Planning Note detour and replaced it with real future-dated #WEEKLY notes plus a rolling scaffold",
        "timeslot_title": "Codex: Weekly note scaffolding",
        "domains": ["me.dev", "dev.nj"],
        "repo": "Notion Journal",
        "session_files": ["rollout-2026-05-04T19-40-48-019df2ca-7899-7692-9a34-3f2dda357872.jsonl"],
        "request": "Scrap the Planning Note flow. Future calendar work should create real future-dated notes with `#WEEKLY`, and the app should keep upcoming weeks scaffolded without a manual import step.",
        "summary": (
            "The calendar flow was changed at the model level instead of polishing the old planning-note UI. "
            "Future dates now create a real note immediately with an initial `#WEEKLY` block, using the target date for note and block creation time while keeping update timestamps current for sync ordering. The separate planning-note sheet, preview, and import path were removed from the active calendar flow. We then added a rolling 30-day scaffold that runs after the initial CloudKit pull, fills any missing future `#WEEKLY` notes idempotently, and confirmed that weekly reconstructed views stay bounded to the current week so the scaffold does not flood the view."
        ),
        "outcome": (
            "Weekly planning is now represented by normal notes instead of a side-channel system. The calendar creates and maintains future weekly structure directly, and the reconstructed weekly view continues to show only the relevant week."
        ),
        "files": [
...
def main() -> None:
    ...
    txt_path = SOURCE_ROOT / "20260504-codex-source.txt"
    json_path = SOURCE_ROOT / "20260504-codex-source.json"
    pdf_path = SOURCE_ROOT / "20260504-codex-source.pdf"
    ...
    source_doc = {
        ...
        "primary_domain": PRIMARY_DOMAIN,
        "source": "Codex session history from local desktop session JSONL files for 2026-05-04",
        "blocks": [
            {
                "topic": b["topic"],
                "repo": b["repo"],
    ...
    existing_slots = cur.execute(
        ...
        """,
        ("%/Users/mac/.codex/sessions/2026/05/04/%",),
    ).fetchall()
    cur.execute(
        """
        ...
        WHERE owner_scope = 'ME' AND deleted = 0 AND title LIKE 'Codex:%' AND notes LIKE ?
        """,
        (latest_end, "%/Users/mac/.codex/sessions/2026/05/04/%"),
    )
